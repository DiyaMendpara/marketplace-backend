...
export class AssistantService {
  ...
  async chat(message: string) {
    ...
    const context = catalog.slice(0, 12).map((p) => `${p.name}: ${p.category}, ₹${p.pricePerMeter}/m, MOQ ${p.moq}m, ${p.description}`).join('\n');
    if (key) {
      try {
    ...
    const reply = matches.length ? `I found ${matches.length} relevant option${matches.length === 1 ? '' : 's'} in the catalog. Compare price, MOQ, and availability below; tell me your target use, budget, and order size for a narrower shortlist.` : 'Tell me the fabric type, intended use, target price per meter, and minimum quantity. I will narrow the current catalog for you.';
